...
export function AcademySidebar({ items }: { items: TocItem[] }) {
          ...
          >
            <span
              className={`text-xs tabular-nums ${
                activeId === item.id ? "text-slate-300" : "text-slate-400"
              }`}
            >
              {item.num}
            </span>
  ...
  return (
    <>
      ...
      {mobileOpen && (
        <nav className="mb-4 rounded-lg border border-slate-200 bg-white p-3 lg:hidden">
          {tocList}
        </nav>
      )}

      <nav className="sticky top-4 hidden max-h-[calc(100vh-2rem)] overflow-y-auto rounded-lg border border-slate-200 bg-white p-3 lg:block">
        <p className="mb-2 px-2.5 text-xs font-semibold uppercase tracking-wide text-slate-400">
          Inhoud
        </p>
        {tocList}
        <Link
          href="#top"
          className="mt-2 block px-2.5 py-1.5 text-xs text-slate-400 hover:text-slate-600"
        >
          ↑ Naar boven
        </Link>
      </nav>
